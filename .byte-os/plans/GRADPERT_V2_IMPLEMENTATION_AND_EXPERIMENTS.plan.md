...
**后续协议覆盖（2026-09-24）：**本文件保留 2026-09-21 的历史规划；其中
50 轮训练、早期宽度与 loss 平均策略等数值不再代表当前基线。现行 Jurkat
v2 默认是 Top500、GenePT-PCA256、SwiGLU、统一 row_mean、全局 batch128、
双卡同步训练 5 轮。容量测试已通过，正式基线已启动；准确运行身份、提交、
配置哈希和监控见 [STATE.md](../STATE.md)。后续消融以
`configs/v2/glm53_flash_jurkat/default/gradpert_v2/nadig_jurkat.yaml` 为父配置，
不能按本历史表格直接发起 50 轮或旧架构运行。
