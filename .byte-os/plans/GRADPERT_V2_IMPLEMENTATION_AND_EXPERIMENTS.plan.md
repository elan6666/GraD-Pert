...
日期：2026-09-21。状态：供用户审阅的执行计划；本轮仅产出计划，没有实现或启动实验。
本计划承接用户明确要求的后续顺序，不把早期“仅讨论”永久解释为禁止未来实现。
具体开始实施与运行以用户后续安排为准。本计划不更改正在运行的v1任务。
...
| B 完整实现 | v2模块、独立训练路径、版本选择、恢复与评估 | 合成契约测试和v1兼容回归通过，源码发布可追溯 |
...
1. d256、query1000、完整双SSL、2G/4L，单卡从microbatch 1开始，逐次2/4/8/16/32…；
...
- 比较每卡1个独立任务与单任务2卡DDP的总吞吐、峰值内存、通信时间。
...
默认先以Nadig Jurkat作开发数据集（本计划建议，尚非用户锁定）；固定seed1筛选，
...
| H1 | 超参数，第一组 | LR 1e-3/1e-4（2行） | 其余固定，沿用GLM5对齐日程 |
...
- 调度依据C实测吞吐和实时占用；优先独立任务并行，最多每卡两任务沿现行许可，不能
  为填满槽位降低门禁。DDP任务明确占用两卡。已有实验不抢占、不杀停。
...
本轮完成计划文档。尚未完成A规格冻结、B实现、C服务器实测、D可运行配置/脚本，
没有启动基线或消融。首个后续工作包是A+B：先消除星号规格与版本兼容风险，再完成
完整方法；C测试结果出来后才给出正式microbatch和执行拓扑，不提前承诺双5090最大batch。
