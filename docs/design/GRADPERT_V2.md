...
2026-09-25 的新运行方法已实现为 Hamiltonian 图传播 + 全量 MLA；完整公式、参数和待审问题见[新方法说明](GRADPERT_V2_HAMILTONIAN_MLA_METHOD.md)。本文件中的早期 Top500/DSA 方案和历史运行须按其各自源码、配置解释。

本文件主体保留 2026-09-21 的历史设计讨论；上方链接的新方法说明记录
2026-09-25 已实现的版本。历史 v1/B2 的正式协议不被本文改写。
...
| GenePT 输入维度 | 后续 v2 默认使用 B1 的 GenePT-only PCA256 表 | 从冻结的 2048 维 GenePT 向量确定性降至 256 维；该表进入模型后可训练，不再叠加 2048→256 的可训练适配层。历史 B0 保留原始 2048 维表与适配层 |
...
NCCL worker 保留短通信超时用于真实梯度/统计 collective。仅主卡执行的完整
validation、test 和文件操作通过独立 Gloo CPU 控制组广播结果，控制组上限为
...
v2 可显式提供 `expression_holdout_path` 与 `expression_holdout_sha256`，必须
成对出现。未提供表示原先完整表达轴协议，保留已经封存的容量配置哈希和行为；
它不是隐式启用留出的默认值。留出配置必须显式封存该清单，不能从目录名推断。
清单由 gene ID 顺序、预注册 heldout_count 与 seed 生成，不读取表达值；加载时
重新核对分区配方、表达轴、清单哈希。训练 query 只能来自 training_gene_ids，
主预测输入/目标及 SSL2 Student/Teacher 所有视图统一从该池抽样。SSL1 可以保留
这些基因的身份/外部图知识，但不接触其表达。运行身份记录清单哈希。
回归测试把被排除基因的 control 和 target 表达改成极大相反值，验证主预测加
两类蒸馏的损失及全部学生梯度不变，即使该基因仍是扰动靶点。
正式 G1 仍需独立训练与评估；不能对普通全轴训练 checkpoint 贴上“留出”标签。

## 16. 双卡实测与首轮配置冻结（2026-09-22）

> 本节容量和首批矩阵使用旧的全表达轴训练协议。第17节用户确认的默认表达排除
> 是后续协议修正；原收据保留原源码身份，不作为新协议已经验收的证据。

采用一个同步双RTX5090任务，world_size=2、accumulation=1；每卡复制模型。
完整双SSL、d256、query1000、2Global/4Local及其余方法设置固定。

| 每卡物理batch | 全局batch | 128步/恢复/推理 | 训练cells/s | 每卡峰值allocated GiB |
|---:|---:|---|---:|---:|
|32|64|通过|8.85|13.92|
|64|128|通过|17.58|26.91|
|74|148|两次通过|20.12（eval128复测）|30.17|
|75|150|20步后OOM|不作成功吞吐比较|失败|

默认工程参考为每卡64、全局128，H3比较全局64/128/148；最大通过点与默认运行点分别记录。
以上是固定128步协议下的相邻边界，不承诺50epoch绝无OOM。推理cell batch128已通过
固定输入数值一致性检查；默认完整表达轴按1000基因分块，5000上下文是独立评估任务。

收据见 `.byte-os/evidence/v2-dual-gpu/`，原训练SHA保留。Jurkat学生共43,808,513参数，
其中四个蒸馏头20,980,736；不含EMA副本及buffer。实际47行完整YAML位于
`configs/v2/preregistered_jurkat/`。B0/H1待最终源码精确预检；后续组是预注册候选，
必须从真实H1→H2→H3验证选择结果重新生成，不能将候选值当作已验证赢家。

Jurkat对应全局64/128/148每epoch分别2076/1081/947次更新；H3固定50epoch，
报告更新数和KoLeo实际近邻集合差异。参考128的步耗时中位数约7.264秒，
50epoch纯更新粗估约109小时，未计完整验证、检查点和其他开销，不是实测总工期。
其他数据集复用同一脚本，但须使用自己的容量收据，不复制Jurkat的容量结论。

## 17. 默认训练表达可见性（2026-09-22 用户确认）

配置位置为 `model.exclude_test_target_expression`，严格布尔值。v2省略时默认true，
v1省略时默认false且不改变旧配置序列化；两个版本均可显式覆盖。

```yaml
model:
  version: v2
  model_id: gradpert_v2
  exclude_test_target_expression: true
  # 其余字段仍须完整提供。
```

排除集合来自冻结test条件的全部非control靶基因与表达轴的交集，仅读取条件身份，
不读取测试响应值。多靶点条件取并集；图轴独有靶点不占表达列。v2的主预测输入、
训练目标以及SSL2 Student/Teacher的所有Global/Local表达视图共用受限候选池。
GenePT、图节点、图邻域与SSL1基因先验计算不因该开关删除。测试允许读取完整control
表达并预测完整输出轴；该训练开关不屏蔽测试输入。

v1没有表达token采样接口。显式true时保留固定宽度和原网络，将排除列的训练control
输入置为固定零值，同时从表达预测损失中选择其余列。默认false保持原输入和损失路径。
运行收据记录开关、测试条件哈希、排除基因清单及哈希、允许表达轴哈希；v2 checkpoint
身份包含该协议，防止跨协议恢复。

G1是独立的额外随机表达列留出，须从默认允许集合中抽样。新holdout-2清单记录
`excluded_from_sampling_gene_ids`；训练最终使用默认允许集合与G1允许集合的交集。
默认排除与G1隐藏集合重叠时拒绝启动，不能把默认排除重复计作额外G1泛化。
旧holdout-1清单仍可用于未启用默认排除的历史协议。

新协议需要新的配置矩阵、运行ID和精确源码预检。全局batch128仍为用户指定默认；
新协议的必要验证不再搜索或扩展batch档位。

## 18. 统一 loss 平均与跨双卡 KoLeo（2026-09-22 用户确认，实施中）

用户最终确认默认 `row_mean`，S1 对照为统一 `condition_mean`。此决定取代第15节
S1仅切换SSL1 condition项、SSL2 iBOT按全局masked token平均、KoLeo局限rank内的旧定义。
两组使用同一采样、视图、loss系数、跨双卡完整有效样本近邻集合及Teacher center更新。

| 项 | 统一策略的应用 |
|---|---|
| 预测MSE | 每行先对基因平均，再统一按行/条件平均 |
| SSL1 condition | 条件表示CE按全局条件频率加权，或条件等权 |
| SSL2 DINO | 每行跨视图CE，再统一按行/条件平均 |
| SSL2 iBOT | 每视图每行先对有效masked token平均，再统一按行/条件平均；空mask行不计入分母 |
| SSL2 KoLeo | 在相同跨双卡完整有效样本集合找近邻，再统一平均每行负log距离 |

固定例外：SSL1 node按有效遮蔽图节点平均；SSL1 spread对去重条件表示计算，不复制
条件表示；Teacher center保持既有目标统计规则，不随loss平均策略变化。

condition_mean先在各条件有效行内平均，再对具有有效行的条件等权平均；row_mean让
各有效行等权。条件计数使用双卡全局集合，不能对两个rank各自condition_mean再平均。
iBOT每个视图有自己的有效行集合；全局没有有效mask的视图不计入视图分母。
两种策略的视图平均规则一致。KoLeo排除自身，近邻可跨条件、跨rank，远端近邻必须
保留梯度；condition_mean仅改变最终权重，不限制近邻的条件范围。

实现验收须覆盖两卡不等长分片、不均条件计数、跨卡最近邻、空mask和全空mask视图，
并与单进程完整有效batch的loss/梯度比较。旧rank内KoLeo容量与旧S1配置不能自动
充当这个新目标的收据；默认物理64/rank、全局128的实际链路须随新目标验证。

## 19. 后续 v2 的 GenePT 默认路线（2026-09-23 用户确认）

新设计及其后续新建的完整 v2 配置默认采用 B1 的 GenePT-only PCA256 路线：
只根据冻结 GenePT 向量和对应数据集的完整有序图基因轴拟合确定性 PCA，
不读取表达、条件或测试结果。降维产物中的 256 维基因向量用作可训练嵌入表；
模型中的 GenePT adapter 在种子维度等于 `d=256` 时为恒等映射。
这不同于在每次前向中训练一个 2048→256 的投影矩阵，也不同于冻结 PCA256 表。

Jurkat B1 的既有产物为
`/data/yilangliu/GraD-Pert/development/v2-genept-pca256-b4e3a08.npz`，SHA256 为
`2460d7757a25aa31f2f06a5c80f1151d2b4fb1a8eba22159658fe62f1ab7cf3d`。
其他数据集应在其各自冻结的图基因轴上生成并核验产物，不直接复用 Jurkat 路径。
历史 B0 及所有已冻结、预注册配置保持原 GenePT 产物和哈希，不追溯修改。
后续配置必须显式写入对应 PCA256 产物路径、哈希和来源；不得依赖隐藏默认值。
此处记录的是后续架构的默认选择，不将已停止的 B0/B1 五轮对照宣称为完整结果。

## 20. GLM-5.3-Flash 启发的无序基因适配（2026-09-23）

新建 Jurkat v2 配置在 B1 可训练 GenePT-PCA256 表上，启用四层中前三层 KDA、
第四层内容索引稀疏 MLA，主干 FFN 使用带 10 裁剪的 SwiGLU。稀疏读取默认
Top500，另列 Top100 对照。每个基因 query 的预算包含自身及 CLS；CLS query
仍可读全部基因。第四层不加入语言模型的因果遮罩、位置编码和连续窗口聚合。
索引分数对被选键加到注意力 logit，使索引器可由任务损失训练。

这些是针对基因无序性的项目适配，并非逐层复刻 GLM-5.3-Flash。前三层 KDA
依赖输入顺序，因此不能宣称整个编码器具有置换不变性。旧 B0/B1 配置及结果
保留原架构，不能与新配置混同。新完整配置在
`configs/v2/glm53_flash_jurkat/default/`，Top100 对照在同族 `top100/`。
目前仅 Jurkat 有核实的 PCA256 产物；其余数据集须单独生成同协议产物。
机制和官方来源见 `docs/provenance/GRADPERT_V2_OPERATORS.md`。

在 6506×256 基因表、其余当前 Jurkat 架构配置下，直接实例化计数：历史 B1
架构为 31,625,217 个可训练参数；新 Top500/SwiGLU 架构为 33,908,867 个。
Top100 只改变检索预算，不改变参数量。这是容量对比，尚非训练收益或速度结论。

首个全局 batch128（每卡微批64、累积1）双卡单步集成预检在反向传播因
32 GB 显存不足失败，0 次更新。后续新配置把每卡微批设为32、累积2，保持
全局 batch128 和科学采样协议；是否通过须以新提交、新运行 ID 的收据为准。
微批32／累积2的第二次单步预检仍在稀疏注意力反向重算时 OOM（0 次更新）。
...
跨细胞四折的 GraD-Pert v2 也采用本节这套最新模型设计（默认 Top500），
