...
状态：2026-09-26 用户已授权开始执行。阶段 A 的方法实现及本地正确性验证完成，正在发布参考版本；后续性能、容量及五轮训练仍待服务器实测。本文覆盖此前粗略计划，不能用历史模型的性能收据替代本方法。
...
图边是 GO/STRING 各入边 Top20、三个种子 1 的双向 Hamiltonian 环和 self 的去重并集，保存四位来源。无虚拟节点。图层 1–3：对**每个目标节点 i**，只在其合法邻域 N(i) 内抽取随机排列，正向写入状态，再从正向末态接力按严格逆序写入。完成双遍后仅目标 i 用自己的 query 读取末态，得到唯一的新 h_i；邻居在该局部扫描里只负责写入。每层的所有目标同步更新，下一层读取上一层的新节点值。第 4 层是仅对 N(i) 读取的稀疏 MLA，保留边来源得分偏置。无全图共享 KDA 状态。图局部视图对节点集合取诱导子图；采样和邻域展开不得泄漏到局部视图外。

Cell 和 Response 各为 2 个双遍接力 self-KDA 块加一个全量、非因果 MLA 块，宽 256、4 头、MLA KV 秩 64、4 个 mHC stream、SwiGLU FFN、dropout 0.1。每个 self-KDA 块随机排列基因，正向写入基因后 CLS 只读不写；反向从该状态继续按逆序写入基因，末尾 CLS 写入一次；全部基因及 CLS 从同一末态读出。下一层状态重新置零。训练时每个视图/样本抽取排列，Cell、Response 和控制交叉读取使用一致的 ID 对齐；验证/测试用固定种子排列并记录种子。随机增强不是数学排列不变性，需报告重排敏感性。

Response 从 Cell 基因状态和独立 response CLS 开始。每层首先通过各层独立的 Linear(512,256)→GELU→Linear(256,256) 将每个 stream 与扰动图表示 e_p 融合，替换入口状态而不另加入口融合残差，再 self，再读取 control 基因状态的 cross，最后 FFN；前两层 cross 使用接力 KDA 的 control K/V 写入与 response Q 末态读取，末层用全量 cross MLA。control CLS 不作为 cross K/V。响应末态基因表示与 e_p 再拼接预测 Δ，yhat=x_control+Δ；SSL2 使用最终 response CLS，SSL1 使用 e_p。不得把扰动真实表达输入预测路径。
...
1. 把上面的合同逐项对应到实现、配置和测试。确认图邻域内写入、仅目标读取、四层同步传播；Cell/Response 的双遍接力、CLS 写入一次、每层清零；逐层扰动注入、self→cross→FFN、独立 response CLS、输出残差头。对“融合 MLP”与当前线性残差注入等实现差异逐项核对，不把能运行等同于符合设计。
