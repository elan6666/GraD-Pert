...
| Masked-node consistency | `ref_dinov2:dinov2/loss/ibot_patch_loss.py:iBOTPatchLoss` | centered teacher targets; student CE on masked positions; per-sample mask normalization | graph nodes replace patch tokens; exactly one global masked | mask-only gradient and center tests |
...
| GEARS training defaults | `ref_gears:gears/gears.py:GEARS.train` and README | train/test batch 32/128; Adam 1e-3, WD 5e-4; best native checkpoint by val DE MSE | common max 200/patience 10 and common validation metric for fairness | isolated-runner config receipt |
...
- Exact independent within-cell raw/processed URLs and checksums for RPE1,
  Jurkat, and HepG2 remain to be resolved before their data configs may become
  `canonical_ready`.
