...
def test_b3_config_combines_reduced_graph_and_all_seven_systems_contract() -> None:
    path = (
        Path(__file__).resolve().parents[2]
        / "configs/pilots/perf_b3_combined/gradpert_b2/nadig_jurkat.yaml"
    )
    config = load_experiment_config(path)
    parameters = config.model.parameters
    assert parameters["performance_pilot_variant"].value == "perf_b3_combined"
    assert parameters["graph_axis_policy"].value == ("recomputed_top500_union_candidate_targets")
    assert parameters["runtime_graph_root"].value == ("pilots/perf_b3_combined/nadig_jurkat")
    assert parameters["systems_optimizations"].value == ("all_seven_semantics_preserving_v1")
    seven = (
        "systems_merged_hdf5_reads",
        "systems_control_expression_cache",
    ...
    assert parameters["systems_prefetch_depth"].value == 2
    assert parameters["systems_log_buffer_steps"].value == 64
    assert config.artifacts.root == "runs/pilots/perf_b3_combined/nadig_jurkat"
    assert config.artifacts.result_mode == "metrics_only"
