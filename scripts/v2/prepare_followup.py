...
def prepare(selection: Path, manifest: Path, parent: Path, group: str, output: Path) -> dict:
    ...
    result = generate(winner, group, output)
...
def main() -> None:
    ...
    result = prepare(args.selection, args.manifest, args.parent, args.group, args.output)
